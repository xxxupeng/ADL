...
class Compose(object):
    ...
    def __call__(self, input, target,position=None):
        if position is not None:
            for t in self.co_transforms:
                input,target,position = t(input,target,position)
...
class RandomCrop(object):
    ...
    def __call__(self, inputs,target,position=None):
        h, w, _ = inputs[0].shape
        th, tw = self.size
        if w < tw: tw=w
        if h < th: th=h
        ...
        if position is not None:
            return inputs, target[y1: y1 + th,x1: x1 + tw],position[y1: y1 + th,x1: x1 + tw]
        else:
